...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        batch_norm_x = np.divide(x - sample_mean, np.sqrt(sample_var + eps))
        out = gamma * batch_norm_x + beta

        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var

        cache = (x, sample_mean, sample_var, batch_norm_x, gamma, beta, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    dsample_var = np.sum(-0.5 * dx_normalized * (x - sample_mean) / (sample_var + eps)**1.5, axis = 0)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(x_normalized * dout, axis = 0)
    dx_normalized = gamma * dout
    dsample_var = np.sum(-0.5 * dx_normalized * x_normalized / (sample_var + eps), axis = 0)
...
def conv_backward_naive(dout, cache):
    ...
    N, C, H, W = x.shape
    F, _, HH, WW = w.shape
    _N, F, H_prime, W_prime = dout.shape
    stride = conv_param['stride']
    pad = conv_param['pad']
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H_prime):
                for j in range(W_prime):
                    x_mask = x[n, c, i*stride : i*stride+HH, j*stride : j*stride+WW]
                    out[n, c, i, j] = np.max(x_mask)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
